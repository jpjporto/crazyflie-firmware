...
static uint32_t frameTime_in_cl_A[LOCODECK_NR_OF_ANCHORS];
static double clockCorrection_T_To_A[LOCODECK_NR_OF_ANCHORS];

static uint8_t currentMasterIdx;
...
#define MAX_DISTANCE_DIFF (15.0f)
...
static uint32_t statsSeqPackets[LOCODECK_NR_OF_ANCHORS];
static uint32_t statsRepeat = 0;
...
static double calcClockCorrection(const uint32_t frameTime, const uint32_t previuosFrameTime) {
    double clockCorrection = 1.0;

    if (frameTime != 0) {
      clockCorrection = ((double)previuosFrameTime / (double)frameTime);
    }
  ...
  if (anchor < LOCODECK_NR_OF_ANCHORS)
  {
    ...
    if (anchor != previousAnchor) 
    {
        ...
        if (isValidRxTime(previuos_rxAr_by_An_in_cl_An) && isValidRxTime(rxAr_by_An_in_cl_An) && isValidRxTime(rxAn_by_Ar_in_cl_Ar)) 
        {
          ...
          const uint32_t frameTime_in_cl_An = truncateToTimeStamp(rxAr_by_An_in_cl_An - previuos_rxAr_by_An_in_cl_An);
    ...
    if(isSeqNrConsecutive(rxPacketBuffer[anchor].Idx, packet->Idx)) // Only move forward if got sequential messages
    {
      statsSeqPackets[anchor]++;
    ...
    if( rxPacketBuffer[anchor].Idx == packet->Idx )
    {
        statsRepeat++;
    }
...
static void Initialize(dwDevice_t *dev, lpsAlgoOptions_t* algoOptions) {
  ...
  memset(statsSeqPackets, 0, sizeof(statsSeqPackets));
  statsRepeat = 0;
}
...
LOG_ADD(LOG_UINT32, repCnt, &statsRepeat)
LOG_ADD(LOG_UINT32, seqCnt0, &statsSeqPackets[0])
LOG_ADD(LOG_UINT32, seqCnt1, &statsSeqPackets[1])
LOG_ADD(LOG_UINT32, seqCnt2, &statsSeqPackets[2])
LOG_ADD(LOG_UINT32, seqCnt3, &statsSeqPackets[3])
LOG_ADD(LOG_UINT32, seqCnt4, &statsSeqPackets[4])
LOG_ADD(LOG_UINT32, seqCnt5, &statsSeqPackets[5])
LOG_ADD(LOG_UINT32, seqCnt6, &statsSeqPackets[6])
LOG_ADD(LOG_UINT32, seqCnt7, &statsSeqPackets[7])
